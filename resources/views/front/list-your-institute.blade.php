...
.subcategory-multi-select input[type="checkbox"] {
    ...
    accent-color: #3b82f6;
    cursor: pointer;
    flex-shrink: 0;           /* Important - checkbox size fixed rahe */
}
...
<section class="list-institute-section">
            ...
            <div class="step-card active" id="step1" style=" margin: auto;">
              ...
              <div class="form-grid">
                <div class="form-group full">
                  ...
                  <input type="text" id="institute_name" placeholder="e.g. Bright Minds Academy" required />
                ...
                <div class="form-group full">
                  ...
                  <div class="input-with-btn">
                    ...
                    <input type="tel" id="mobile" placeholder="10-digit number" maxlength="10" required />
                    <button type="button" class="otp-btn" id="otp-btn" onclick="sendOtp()" style="display:none;">Send OTP</button>
                ...
                <div class="form-group full otp-group" style="display:none;">
                  ...
                  <button type="button" class="otp-btn resend-otp-btn" onclick="sendOtp()">Resend OTP</button>
                  </div>
    ...
    <small class="error-text" id="error_subcategory_id"></small>
                ...
                <div class="form-group full">
                  ...
                  <textarea id="description" rows="3" placeholder="e.g. Best CBSE & ICSE coaching with 95%+ results..."
            ...
            <div class="step-card" id="step3" style="display:none;">
              ...
              <div class="pricing-cards-container grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 max-w-6xl mx-auto desktop-view-card">
        ...
        @endif
             ...
             <div class="pricing-cards-container grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 max-w-6xl mx-auto mobile-view-card">
    @if(isset($packages) && count($packages) > 0)
    @foreach($packages as $package)

        @if($package->is_popular)
        <!-- ==================== MOST POPULAR CARD ==================== -->
        <div class="pricing-card relative bg-gradient-to-b from-white to-blue-50 rounded-2xl border-2 border-blue-500 shadow-2xl p-8 flex flex-col transform scale-105 transition-all duration-300 hover:scale-110 z-10">
            <!-- Recommended Badge -->
            <div class="absolute -top-4 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-sm font-bold px-6 py-1.5 rounded-full shadow-lg">
                Most Popular
            </div>
        ...
        @else
        <!-- ==================== NORMAL CARD ==================== -->
        <div class="pricing-card bg-white rounded-2xl border border-gray-200 shadow-lg p-8 flex flex-col transition-all duration-300 hover:shadow-xl">
            <div class="text-center mb-6">
                <h5 class="text-2xl font-bold text-gray-800">{{$package->name}}</h5>
                <p class="text-sm text-gray-500 mt-1">{{$package->title}}</p>
                <div class="mt-4 text-4xl font-bold text-gray-900">
...
function loadSubCat(cateID, selectedSubCat = null) {
    ...
    $.get(url, function(response) {
        if (response && Array.isArray(response) && response.length > 0) {
            ...
            container.html('<p class="text-gray-500 py-6 text-center">No sub categories found for this category.</p>');
            wrapper.show();
        }
    });
}
...
function saveStep1(){
  clearErrors();
    ...
    let step1url = "{{ url('/step1-save') }}";
    $.post(step1url,data,function(res){
