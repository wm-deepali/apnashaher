...
.active-check{
                        ...
                        <div>
                          <form id="instituteForm" enctype="multipart/form-data">
                            ...
                            <div class="grid md:grid-cols-2 gap-6">
                                <input type="text" name="name" value="{{ $institute->name }}" class="input" placeholder="Institute Name">
                                <input type="text" name="owner_name" value="{{ $institute->owner_name }}" class="input" placeholder="Owner Name">
  ...
  <p class="text-xs text-gray-500 mt-1">
      Example: https://www.xyz.com
                                ...
                                <input  type="text" name="country" value="{{ $institute->country ?? '' }}" class="input" placeholder="Country">
                                <select id="state_id" name="state_id" class="input" required>
                                  <option value="">Select State</option>
                                  @if(isset($states) && count($states) > 0)
                                  @foreach($states as $state)
                                  <option value="{{$state->id}}" {{$institute->state_id == $state->id ? 'selected' : ""}}>{{$state->name}}</option>
                                  @endforeach
                                  @endif
                                  <!-- Add more -->
                                </select>
                                ...
                                <input type="text" name="zipcode" value="{{ $institute->zipcode ?? '' }}" class="input" placeholder="Pin Code">
                            </div>
                                    ...
                                    <div class="timing-card">
                                        ...
                                        <div class="timing-row">
                                            <div class="timing-time">
                                                <input type="time" class="input" name="timings[{{ $day }}][open_time]" value="{{ $timing->open_time }}">
                                                <span>to</span>
                                                <input type="time" class="input" name="timings[{{ $day }}][close_time]" value="{{ $timing->close_time }}">
                                            </div>
...
<div id="courses" class="tab-content hidden space-y-10">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="md:col-span-2">
      ...
      <textarea name="short_desc" class="input w-full h-24" placeholder="Brief overview of the course..." required></textarea>
     <span class="error-text text-red-600 text-sm mt-1 block"></span>
    </div>
    ...
    <div class="md:col-span-2">
      ...
      <textarea name="detailed_information" class="input w-full h-32" placeholder="Full syllabus, topics, outcomes..."></textarea>
     <span class="error-text text-red-600 text-sm mt-1 block"></span>
    </div>
    ...
    <div class="md:col-span-2">
      ...
      <input type="file" name="thumb_image" accept="image/*" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
    ...
    <div class="md:col-span-2 flex justify-end mt-4">
      <button type="submit" class="bg-blue-600 text-white px-8 py-3 rounded-xl font-medium hover:bg-blue-700 transition shadow-md">
        Add Course
      </button>
    </div>
  </div>
...
<div class="bg-white rounded-2xl shadow-md border border-gray-200 p-6 md:p-8">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
   ...
   <!-- Professional Course Card -->
    <div class="course-card group bg-white rounded-2xl overflow-hidden border border-gray-200 shadow-md hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
      <!-- Thumbnail -->
      <div class="relative">
        ...
        <img src="https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&q=80&w=800" 
             alt="Course Thumbnail" class="w-full h-52 object-cover transition-transform duration-500 group-hover:scale-105">
        @endif
             <span class="absolute top-4 right-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-xs font-bold px-4 py-1.5 rounded-full shadow-md">
          Active
        </span>
      </div>
      ...
      <div class="p-6 space-y-5">
        ...
        <div class="inline-block bg-gradient-to-r from-blue-50 to-cyan-50 px-4 py-1.5 rounded-full text-sm font-medium text-blue-800 border border-blue-100">
        ...
        <div class="flex items-center justify-between pt-2">
          <div>
            ...
            <p class="text-base font-semibold text-gray-900">{{ $course->duration }} {{ $course->duration_unit }}</p>
          </div>
        ...
        <div class="pt-4 flex items-center justify-between border-t border-gray-100">
          ...
          <div class="flex gap-5">
            ...
            <button 
                data-id="{{ $course->id }}" 
                data-url="{{ route('institute.courses.destroy', $course->id) }}"
...
<div id="gallery" class="tab-content hidden space-y-10">
    ...
    <div class="space-y-6">
      <div>
        ...
        <input type="file"  id="images"
          name="images[]" multiple accept="image/jpeg,image/png,image/webp,image/avif" class="block w-full text-sm text-gray-500 
  ...
  <div class="bg-white rounded-2xl shadow-md border border-gray-200 p-6 md:p-8">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
      ...
      <div class="relative group overflow-hidden rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer">
        <img src="{{ asset('storage/'.$gallery->image) }}" 
            class="w-full h-48 object-cover transition-transform duration-500 group-hover:scale-105">
        <button data-id="{{ $gallery->id }}" class="delete-gallery-btn absolute top-2 right-2 bg-red-500 text-white text-xs px-3 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-md">
          Delete
        </button>
      </div>
    @empty
...
<div id="lightbox" class="fixed inset-0 bg-black/90 hidden z-50 flex items-center justify-center">
  <button onclick="closeLightbox()" class="absolute top-6 right-6 text-white text-4xl font-light hover:text-gray-300 transition">&times;</button>
  
  <button onclick="prevImage()" class="absolute left-6 text-white text-5xl hover:text-gray-300 transition">&lsaquo;</button>
  <button onclick="nextImage()" class="absolute right-6 text-white text-5xl hover:text-gray-300 transition">&rsaquo;</button>
  
  <img id="lightboxImg" src="" class="max-w-[90%] max-h-[90%] object-contain rounded-lg shadow-2xl" alt="Gallery Image">
...
<div id="leads" class="tab-content hidden flex flex-col h-full w-full bg-gray-50 overflow-hidden">
  ...
  <div id="leadsTableContainer" class="flex-1 bg-white overflow-y-auto transition-all duration-400 ease-in-out" style="background: #4325e712;">
        ...
        <table class="w-full text-sm min-w-full divide-y divide-gray-200 table-fixed">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-4 text-left font-semibold text-gray-700 uppercase tracking-wider w-1/5 hidden sm:table-cell">Phone</th>
  ...
  <div id="leadDetailDrawer"
    ...
    <!-- Drawer Header -->
    <div class="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-gray-200/50 px-6 py-5 flex items-center justify-between">
    ...
    <div class="p-6 space-y-8">
      ...
      <div class="grid grid-cols-2 gap-4">
        <button class="bg-green-600 text-white py-3.5 rounded-xl font-medium hover:bg-green-700 transition shadow-md flex items-center justify-center gap-2">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
          Call Now
        </button>
        <button class="bg-green-500 text-white py-3.5 rounded-xl font-medium hover:bg-green-600 transition shadow-md flex items-center justify-center gap-2">
      ...
      <table class="notif-table w-full text-sm">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-4 text-left font-semibold text-gray-700 uppercase tracking-wider w-1/2 hidden md:table-cell">Message</th>
        ...
        <tbody class="divide-y divide-gray-200">
          ...
          @endphp
            <tr class="notif-row hover:bg-blue-50/50 cursor-pointer transition-colors"
                ...
                <td class="px-6 py-4">
                  <span class="notif-status {{ $statusClass }} px-3 py-1 rounded-full text-xs font-medium">{{ $status }}</span>
                </td>
            </tr>
            @empty
            <tr>
...
<div id="notifModal" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 hidden">
  <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full mx-4 overflow-hidden">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white flex justify-between items-center">
      ...
      <button onclick="closeNotifModal()" class="text-white text-3xl leading-none hover:text-gray-200">&times;</button>
    </div>
    ...
    <div class="p-6 border-t bg-gray-50 flex justify-end">
      <button onclick="closeNotifModal()" class="px-8 py-3 bg-gray-200 text-gray-800 rounded-xl hover:bg-gray-300 transition">
        Close
      </button>
    </div>
  </div>
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-4 mb-6">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600" style="font-weight: 700;">{{date('d F Y', strtotime($review->created_at))}}</p>
                                </div>
                            </div>
  ...
  <div class="max-w-4xl mx-auto">
   ...
   {{-- <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden relative">
      <!-- Popular Badge (top-right corner) -->
       ...
       @endphp
      <div class="absolute top-0 right-0 bg-gradient-to-r from-orange-400 to-orange-500 text-white text-xs font-bold px-6 py-1.5 rounded-bl-xl shadow-md transform rotate-12 translate-x-4 translate-y-2">
      ...
      <div class="bg-gradient-to-r from-blue-600 to-indigo-700 p-8 text-white">
        ...
        <!-- Current Plan Badge -->
        <div class="mt-6 inline-block bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full text-white font-medium text-sm border border-white/30">
      ...
      <div class="p-8">
        <ul class="space-y-4 text-gray-700">
          <li class="flex items-center gap-3">
            ...
            <span>{{$features->apnashaher_listing ? 'ApnaShaher Listing' : ''}} + {{ucfirst($features->search_visibility)}}Search Visibility</span>
          </li>
          <li class="flex items-center gap-3">
            <svg class="w-5 h-5 text-green-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
            </svg>
            <span>{{ucfirst($features->contact_display ?? "")}} Contact Details Display {{$features->call_whatsapp_button ? '+ WhatsApp/Call CTA' : ''}}</span>
          </li>
          <li class="flex items-center gap-3">
            <svg class="w-5 h-5 text-green-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
            </svg>
            <span>{{ucfirst($features->profile_editing ?? "")}} Profile Editing {{$features->preferred_institute_badge ? '+ Preferred Institute Badge' : ''}}</span>
          </li>
           ...
           <li class="flex items-center gap-3">
            ...
            <span>Up to {{$features->courses_programs}} Courses/Programs {{$features->verified_badge ? '+ Verified Badge' : ''}}</span>
          </li>
          ...
          <li class="flex items-center gap-3">
            ...
            <span>{{$features->promotional_banner_placement ? 'Preferred Placement' : ''}} {{$features->profile_performance_insight ? '+ Profile Analytics' : ''}}</span>
          </li>
          @endif
          ...
          <li class="flex items-center gap-3">
            ...
            <span>{{$features->ai_profile_description_generator ? 'AI Description' : ''}} {{$features->custom_profile_url ? '+ Custom Profile URL' : ''}}</span>
        ...
        <div class="mt-10 pt-6 border-t border-gray-200">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p class="text-xl font-semibold text-gray-900">{{ \Carbon\Carbon::parse($plan->expiry_date)->format('d M Y') }}</p>
            </div>
            <a href="{{route('plans')}}" class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-3 rounded-xl font-medium hover:from-blue-700 hover:to-indigo-700 transition-all shadow-md">
   ...
   <div class="bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden relative">
    ...
    <!-- Popular Badge -->
    @if($isPopular)
    <div class="absolute top-0 right-0 bg-gradient-to-r from-orange-400 to-orange-500 text-white text-xs font-bold px-6 py-1.5 rounded-bl-2xl shadow-md transform rotate-12 translate-x-3 translate-y-2">
        POPULAR
    </div>
    @endif
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 p-8 text-white">
        ...
        <!-- Current Plan Status -->
        <div class="mt-6 inline-block bg-white/20 backdrop-blur-sm px-5 py-2 rounded-full text-white font-medium text-sm border border-white/30">
            Current Plan • 
    ...
    <div class="p-8">
        ...
        <ul class="space-y-4 text-gray-700">
            <li class="flex items-center gap-3">
                ...
                <span>ApnaShaher Listing + {{ ucfirst($features->search_visibility ?? 'Standard') }} Search Visibility</span>
            </li>
        ...
        <div class="mt-10 pt-6 border-t border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <a href="{{ route('plans') }}" 
                   class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-8 py-3.5 rounded-2xl font-semibold shadow-md transition-all">
                    Upgrade Plan →
                </a>
            </div>
        </div>
...
<div id="settings" class="tab-content hidden ">
  <div class="max-w-3xl mx-auto space-y-8">
    ...
    <div class="text-center md:text-left">
      ...
      <p class="text-gray-600 mt-1">Manage your contact information securely. OTP verification required for mobile & WhatsApp changes.</p>
    </div>
        ...
        <div class="p-6">
          <div class="flex items-start justify-between">
            <div>
              ...
              <p id="currentEmail" class="text-gray-700 mt-1 text-xl font-medium">{{ $institute->owner_email ?? "" }}</p>
            </div>
        ...
        <div class="p-6">
          <div class="flex items-start justify-between">
            <div>
              ...
              <p id="currentWhatsApp" class="text-gray-700 mt-1 text-xl font-medium"> +91 {{ $institute->whatsapp }}</p>
            </div>
  ...
  <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden transform transition-all scale-95">
    ...
    <div class="p-6 space-y-6">
      ...
      <div>
        ...
        <input id="modalInput" type="text" class="mt-1 block w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="Enter new number/email">
      </div>
      ...
      <div id="otpSection" class="hidden space-y-4">
        ...
        <div class="flex gap-3">
          <input type="text" maxlength="1" class="w-12 h-12 text-center text-xl border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
          <input type="text" maxlength="1" class="w-12 h-12 text-center text-xl border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
          <input type="text" maxlength="1" class="w-12 h-12 text-center text-xl border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
          <input type="text" maxlength="1" class="w-12 h-12 text-center text-xl border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
        </div>
        <p class="text-xs text-gray-500">OTP sent to your current number/email. Didn't receive? <button onclick="resendOtpRequest()" id="resendBtnRequest" class="text-blue-600 hover:underline">Resend</button></p>
        <span id="Requesttimer" class="ml-2 text-gray-400"></span>
      </div>

      <!-- Action Buttons -->
      <div class="flex gap-4 pt-4">
        <button onclick="closeModal()" class="flex-1 py-3 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50">
          Cancel
        </button>
        <button id="actionBtn" onclick="handleAction()" class="flex-1 py-3 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition">
          Send OTP
        </button>
      </div>
